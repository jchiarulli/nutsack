...
var d = createDebug("ndk-wallet:nutzap-monitor");
var NDKNutzapMonitor = class extends EventEmitter {
  ndk;
  user;
  relaySet;
  sub;
  ...
  findWalletForNutzap(nutzap) {
    ...
    if (!wallet) {
      ...
      wallet = this.allWallets.find((w) => w.mints.map(normalizeUrl).includes(normalizedMint));
      if (!wallet)
...
import { NDKPool, NDKPrivateKeySigner as NDKPrivateKeySigner2 } from "@nostr-dev-kit/ndk";

// src/wallets/nwc/req.ts
import { NDKEvent as NDKEvent2, NDKKind as NDKKind3 } from "@nostr-dev-kit/ndk";

// src/wallets/nwc/res.ts
import { NDKKind as NDKKind2 } from "@nostr-dev-kit/ndk";
async function waitForResponse(requestId) {
  ...
  return new Promise((resolve, reject) => {
    const sub = this.ndk.subscribe(
      {
        kinds: [NDKKind2.NostrWalletConnectRes],
        "#e": [requestId],
        limit: 1
      },
...
async function sendReq(method, params) {
  ...
  const event = new NDKEvent2(this.ndk, {
    kind: NDKKind3.NostrWalletConnectReq,
...
var d2 = createDebug2("ndk-wallet:nwc");
var NDKNWCWallet = class extends EventEmitter2 {
  type = "nwc";
  ...
  async init(pubkey, relayUrls, secret) {
    ...
    this.signer = new NDKPrivateKeySigner2(secret);
...
import { NDKEvent as NDKEvent8, NDKKind as NDKKind10, NDKPrivateKeySigner as NDKPrivateKeySigner3, NDKRelaySet as NDKRelaySet3, normalizeUrl as normalizeUrl5 } from "@nostr-dev-kit/ndk";

// src/wallets/cashu/token.ts
import { NDKEvent as NDKEvent3, NDKKind as NDKKind4, normalizeUrl as normalizeUrl2 } from "@nostr-dev-kit/ndk";

// src/wallets/cashu/decrypt.ts
import createDebug3 from "debug";
var debug = createDebug3("ndk-wallet:cashu:decrypt");
...
var NDKCashuToken = class _NDKCashuToken extends NDKEvent3 {
  _proofs = [];
  original;
  constructor(ndk, event) {
    super(ndk, event);
    this.kind ??= NDKKind4.CashuToken;
  }
  ...
  set mint(mint) {
    this.removeTag("mint");
    this.tags.push(["mint", normalizeUrl2(mint)]);
  }
  get mint() {
    const t = this.tagValue("mint");
    if (t)
      return normalizeUrl2(t);
  }
  get amount() {
...
import { NDKKind as NDKKind5 } from "@nostr-dev-kit/ndk";
import { NDKEvent as NDKEvent4 } from "@nostr-dev-kit/ndk";
...
// src/wallets/cashu/quote.ts
var NDKCashuQuote = class _NDKCashuQuote extends NDKEvent4 {
  quoteId;
  mint;
  amount;
  unit;
  _wallet;
  static kind = NDKKind5.CashuQuote;
  constructor(ndk, event) {
    super(ndk, event);
    this.kind ??= NDKKind5.CashuQuote;
  }
  static async from(event) {
...
import { NDKEvent as NDKEvent5, NDKKind as NDKKind6 } from "@nostr-dev-kit/ndk";
import createDebug4 from "debug";
var d3 = createDebug4("ndk-wallet:wallet-change");
var MARKERS = {
  REDEEMED: "redeemed",
  CREATED: "created",
  DESTROYED: "destroyed",
...
var NDKWalletChange = class _NDKWalletChange extends NDKEvent5 {
  static MARKERS = MARKERS;
  static kind = NDKKind6.WalletChange;
  static kinds = [NDKKind6.WalletChange];
  constructor(ndk, event) {
    super(ndk, event);
    this.kind ??= NDKKind6.WalletChange;
  }
  static async from(event) {
...
async function createOutTxEvent(wallet, paymentRequest, paymentResult, updateStateResult) {
  ...
  if (paymentRequest.target) {
    ...
    if (!(paymentRequest.target instanceof NDKUser3)) {
...
// src/wallets/cashu/deposit.ts
var d4 = createDebug5("ndk-wallet:cashu:deposit");
function randomMint(wallet) {
  const mints = wallet.mints;
  const mint = mints[Math.floor(Math.random() * mints.length)];
  return mint;
}
var NDKCashuDeposit = class _NDKCashuDeposit extends EventEmitter3 {
  mint;
  amount;
  quoteId;
  wallet;
...
import { NDKEvent as NDKEvent6, NDKKind as NDKKind7 } from "@nostr-dev-kit/ndk";

// src/wallets/cashu/mint.ts
import { CashuWallet as CashuWallet2, CashuMint as CashuMint2 } from "@cashu/cashu-ts";
...
async function walletForMint(mint, unit, pk, timeout = 5e3) {
  ...
  const wallet = new CashuWallet2(new CashuMint2(mint), { unit, bip39seed: pk });
...
// src/wallets/cashu/validate.ts
var d5 = createDebug6("ndk-wallet:cashu:validate");
...
async function consolidateMintTokens(mint, tokens, wallet) {
  ...
  const deleteEvent = new NDKEvent6(wallet.ndk, { kind: NDKKind7.EventDeletion });
...
import { NDKKind as NDKKind8 } from "@nostr-dev-kit/ndk";
...
var NDKCashuDepositMonitor = class extends EventEmitter4 {
...
import { normalizeUrl as normalizeUrl3 } from "@nostr-dev-kit/ndk";
...
function findMintsInCommon(mintCollections) {
  ...
  for (const mints of mintCollections) {
    for (const mint of mints) {
      const normalizedMint = normalizeUrl3(mint);
...
import { NDKEvent as NDKEvent7, NDKKind as NDKKind9, normalizeUrl as normalizeUrl4 } from "@nostr-dev-kit/ndk";
var WalletState = class {
  ...
  async update(change) {
    ...
    if (newState.deletedTokenIds.size > 0) {
      const deleteEvent = new NDKEvent7(this.wallet.ndk, {
        kind: NDKKind9.EventDeletion,
        tags: [
          ["k", NDKKind9.CashuToken.toString()],
...
// src/wallets/cashu/wallet/index.ts
var d6 = createDebug7("ndk-wallet:cashu:wallet");
var NDKCashuWallet = class _NDKCashuWallet extends EventEmitter5 {
  type = "nip-60";
  /**
  ...
  ndk;
  status = "initial" /* INITIAL */;
  static kind = NDKKind10.CashuWallet;
  static kinds = [NDKKind10.CashuWallet];
  privateTags = [];
  publicTags = [];
  _event;
  walletId = "";
  ...
  constructor(ndk, event) {
    ...
    this.ndk = ndk;
    if (!event) {
      event = new NDKEvent8(ndk);
      event.kind = NDKKind10.CashuWallet;
  ...
  start(opts) {
    ...
    const filters = [
      { kinds: [NDKKind10.CashuToken], authors: [pubkey], ...this.event?.filter() },
      { kinds: [NDKKind10.WalletChange], authors: [pubkey] },
      { kinds: [NDKKind10.CashuQuote], authors: [pubkey] },
      { kinds: [NDKKind10.EventDeletion], authors: [pubkey], "#k": [NDKKind10.CashuToken.toString()] }
    ];
    if (this.event) {
  ...
  async getP2pk() {
    ...
    let signer;
    if (this.privkey) {
      signer = new NDKPrivateKeySigner3(this.privkey);
    } else {
      signer = NDKPrivateKeySigner3.generate();
  ...
  get relaySet() {
    ...
    if (this.relays.length === 0)
      return void 0;
    return NDKRelaySet3.fromRelayUrls(this.relays, this.ndk);
  }
  /**
   * Prepares a deposit
   * @param amount
  ...
  async redeemNutzap(nutzap, { onRedeemed, onTxEventCreated }) {
    ...
    if (nutzap.p2pk === user.pubkey) {
      if (this.ndk.signer instanceof NDKPrivateKeySigner3)
...
import { NDKKind as NDKKind11 } from "@nostr-dev-kit/ndk";
async function getCashuMintRecommendations(ndk, filter) {
  const f = [
    { kinds: [NDKKind11.EcashMintRecommendation], "#k": ["38002"], ...filter || {} },
    { kinds: [NDKKind11.CashuMintList], ...filter || {} }
  ];
  const res = {};
  const recommendations = await ndk.fetchEvents(f);
  for (const event of recommendations) {
    switch (event.kind) {
      case NDKKind11.EcashMintRecommendation:
...
import { CashuMint as CashuMint3, CashuWallet as CashuWallet3 } from "@cashu/cashu-ts";
var NDKLnPay = class {
  wallet;
  info;
  type = "ln";
  ...
  async payNut() {
    ...
    const quotesPromises = mints.map(async (mint2) => {
      const wallet2 = new CashuWallet3(new CashuMint3(mint2), { unit });
    ...
    const wallet = new CashuWallet3(new CashuMint3(mint), { unit });
...
// src/wallets/webln/index.ts
var NDKWebLNWallet = class extends EventEmitter6 {
  type = "webln";
  walletId = "webln";
  status = "initial" /* INITIAL */;
  provider;
