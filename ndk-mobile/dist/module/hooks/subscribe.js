...
import { NDKRelaySet, wrapEvent } from '@nostr-dev-kit/ndk';
 ...
 */
const createSubscribeStore = (bufferMs = 30) => createStore((set, get) => {
...
export const useSubscribe = ({
  ...
  const store = useMemo(() => createSubscribeStore(opts?.bufferMs), [filters]);
  ...
  const handleEvent = useCallback(event => {
    ...
    if (opts?.wrap) event = wrapEvent(event);
    event.once("deleted", () => {
