...
export class NDKCacheAdapterSqlite {
  ...
  async initialize() {
    ...
    let {
      user_version: schemaVersion
    } = this.db.getFirstSync(`PRAGMA user_version;`);
    if (!schemaVersion) {
      schemaVersion = 0;
      ...
      for (const filter of subscription.filters) {
        ...
        for (const key in filter) {
          if (key.startsWith('#') && key.length === 2) {
            const tag = key[1];
            const events = this.db.getAllSync(`SELECT * FROM events INNER JOIN event_tags ON events.id = event_tags.event_id WHERE event_tags.tag = ? AND event_tags.value IN (${filter[key].map(() => '?').join(',')})`, [tag, ...filter[key]]);
  ...
  async setEvent(event, filters, relay) {
    this.onReady(async () => {
      ...
      if (event.isReplaceable()) {
        await Promise.all([this.db.runAsync(`DELETE FROM events WHERE id = ?;`, [event.id]), this.db.runAsync(`DELETE FROM event_tags WHERE event_id = ?;`, [event.id])]);
        return;
      }
      this.db.runAsync(`INSERT INTO events (id, created_at, pubkey, event, kind, relay) VALUES (?, ?, ?, ?, ?, ?);`, [event.id, event.created_at, event.pubkey, event.serialize(true, true), event.kind, relay?.url || '']);
      const filterTags = event.tags.filter(tag => tag[0].length === 1).map(tag => [tag[0], tag[1]]);
      if (filterTags.length < 10) {
        filterTags.forEach(tag => this.db.runAsync(`INSERT INTO event_tags (event_id, tag, value) VALUES (?, ?, ?);`, [event.id, tag[0], tag[1]]));
      }
  ...
  fetchProfileSync(pubkey) {
    ...
    let result = null;
    try {
      result = this.db.getFirstSync(`SELECT profile, catched_at FROM profiles WHERE pubkey = ?;`, [pubkey]);
    } catch (e) {
  ...
  async fetchProfile(pubkey) {
    ...
    return await this.ifReady(async () => {
      const result = await this.db.getFirstAsync(`SELECT profile, catched_at FROM profiles WHERE pubkey = ?;`, [pubkey]);
      if (result) {
        try {
          const profile = JSON.parse(result.profile);
          const entry = {
  ...
  addUnpublishedEvent(event, relayUrls) {
    ...
    this.onReady(async () => {
      ...
      try {
        this.db.runSync(`INSERT INTO unpublished_events (id, event, relays, last_try_at) VALUES (?, ?, ?, ?);`, [event.id, event.serialize(true, true), JSON.stringify(relayStatus), Date.now()]);
  ...
  async fetchWot() {
    return await this.onReady(async () => {
      const wot = await this.db.getAllAsync(`SELECT * FROM wot`);
